...
void MainWindow::initNetInfo()
{
    ...
    // 读数据 - secnode
    QVector<QVariantList> list = m_dbop.selectDatabase("secnode");
    // 设置行数
    ...
    for(int i=0; i<list.size(); ++i)
    {
        ...
        ui->tableWidget->setItem(i, column++, new QTableWidgetItem(sub.at(0).toString()));
        ui->tableWidget->setItem(i, column++, new QTableWidgetItem(sub.at(1).toString()));
        ui->tableWidget->setItem(i, column++, new QTableWidgetItem(sub.at(3).toDateTime().toString("yyyy-MM-dd hh:mm:ss")));
        ui->tableWidget->setItem(i, column++, new QTableWidgetItem(QString::number(sub.at(4).toInt())));
        ui->tableWidget->setItem(i, column++, new QTableWidgetItem(QString::number(sub.at(5).toInt())));
    }
...
void MainWindow::on_vagueSearch_clicked()
{
    ...
    QVector<QVariantList> list = m_dbop.selectDatabase("secnode", sql);
    // 清空列表
    ...
    {
        QVariantList sub = list.at(i);
        ui->tableWidget->setItem(i, 0, new QTableWidgetItem(QString::number(sub.at(0).toInt())));
