...
int TcpSocket::recvMsg(char** recvData, int& recvLen, int timeout)
{
	...
	int netdatalen = 0;
	int n = readn(&netdatalen, 4);//读包头4个字节-数据长度
	if (n == -1)
	{
		//printf("func readn() err:%d \n", ret);
		m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, n, "func readn() err");
		return n;
	}
	else if (n < 4)//对方异常关闭错误 
	{
		ret = PeerCloseError;
	...
	n = readn(tmpBuf, datalen);//根据长度读数据
	if (n == -1)
	{
		m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, n, "readn() err");
		return n;
	}
	else if (n < datalen)
	{
