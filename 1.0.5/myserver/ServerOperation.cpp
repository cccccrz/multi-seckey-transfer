...
int ServerOperation::secKeyAgree(RequestMsg* reqMsg, char** outData, int& dataLen)
{
	...
	cout << "授予seckeyID:" << rspMsg.seckeyID << endl;
...
int ServerOperation::secKeyCheck(RequestMsg* reqMsg, char** outData, int& dataLen)
{
	...
	//传入seckeyid 传出seckey
	m_occi.get_secKey(keyID, &seckey, seckeyLen);
	//cout << "[OP]seckey:==" << seckey << endl;
	//cout << seckeyLen << endl;
	char buftmp[128] = { 0 };
	strcpy(buftmp, seckey);
	//cout << "buf--Len:==" << strlen(buftmp) << endl;
	//哈希运算 比较reqMsg->authCode
	//检验消息认证码--HMAC  @serverID+clientID@,seckey 
	char key[1024];
	unsigned int mdlen;
	...
	//cout << "[key]==" << key << endl;
	HMAC(EVP_sha256(), key, strlen(key), \
		(unsigned char*)buftmp, seckeyLen, md, &mdlen);
	//cout << "md:" << md << endl;
	//cout << "mdlen:" << mdlen << endl;
	for (int i = 0; i < SHA256_DIGEST_LENGTH; i++)
	{
		sprintf(&key[2 * i], "%02x", md[i]);
	}
	//cout << "服务端消息认证码：" << key << endl;
	//cout << "客户端消息认证码：" << reqMsg->authCode << endl;
	if (strcmp(key, reqMsg->authCode) != 0)
...
int ServerOperation::secKeyRevoke(RequestMsg* reqMsg, char** outData, int& dataLen)
{
	...
	{
		//删除秘钥信息
		m_occi.delete_SecKeyInfo(reqMsg->clientID, reqMsg->serverID);
		rspMsg.rv = 0;
	}
	rspMsg.seckeyID = 0;
	strcpy(rspMsg.clientID, reqMsg->clientID);
...
void* working(void* arg)
{
	...
	case 2:
		...
		break;
	case 3:
		op->secKeyRevoke(reqMsg, &sendData, dataLen);
		break;
	case 4:
		//秘钥查看
