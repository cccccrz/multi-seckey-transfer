...
//根据 clientID 和 serverID 查找秘钥ID
int OCCIOP::view_SecKeyID(char* clientID, char* serverID)
{
	m_seckeyID.clear();

	string sql = " select * from SECMNG.SECKEYINFO where serverid=:1 and clientid=:2";
	Statement* stmt = m_conn->createStatement(sql);
	...
	{
		m_seckeyID.push_back(resSet->getInt(3));//存入数组
	}
...
int OCCIOP::delete_SecKeyInfo(int seckeyID)
{
	...
	string sql = " delete from SECMNG.SECKEYINFO where keyid=" + to_string(seckeyID);
	Statement* stmt = m_conn->createStatement(sql);
	//设置自动提交
	stmt->setAutoCommit(true);
	//执行
...
int OCCIOP::get_secKey(int seckeyID, char** seckey, int& dataLen)
{
	int ret = 0;
	char buf[1024] = { 0 };
	string sql = "select * from SECMNG.SECKEYINFO where keyid= " + to_string(seckeyID);
	Statement* stmt = m_conn->createStatement(sql);
	ResultSet* resSet = stmt->executeQuery();
	if (resSet->next())
	{
		strcpy(buf, resSet->getString(6).data());
	}
	else
	{
		ret = -1;
