...
int ClientOperation::secKeyCheck()
{
	...
	RequestMsg reqMsg;
	memset(&reqMsg, 0x00, sizeof(RequestMsg));
	reqMsg.cmdType = RequestCodec::CHECK;
	strcpy(reqMsg.clientID, m_info.clientID);
	strcpy(reqMsg.serverID, m_info.serverID);
	...
	//r1 存放秘钥ID
	//itoa(reqMsg.r1, node.seckeyID, 10);
	sprintf(reqMsg.r1, "%d", node.seckeyID);
	//cout << "[seckey]:" << node.seckey << endl;
	//cout << strlen(node.seckey) << endl;
	//使用HMAC 生成哈希值--消息认证码 @serverid+clientid@  , seckey
	char key[64];
	unsigned int mdlen;
	...
	//cout << "[key]==" << key << endl;
	HMAC(EVP_sha256(), key, strlen(key), \
		(unsigned char*)node.seckey, strlen(node.seckey), md, &mdlen);
	//cout << "md:" << md << endl;
	//cout << "mdlen:" << mdlen << endl;
	for (int i = 0; i < SHA256_DIGEST_LENGTH; i++)
	{
